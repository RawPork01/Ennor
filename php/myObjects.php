...
$daten = array();
$numRows = 0;
$sql="SELECT s_id, s_name, s_beschreibung, s_besitzDatum, s_datumOnline, s_bild, tbl_benutzer.b_benutzername FROM tbl_sammlerobjekt INNER JOIN tbl_benutzer ON tbl_sammlerobjekt.s_benutzer = tbl_benutzer.b_id WHERE b_id = ? ORDER BY s_datumOnline DESC LIMIT 9";
$statement = $db->prepare($sql);
$statement->bind_param('i', $_SESSION['b_id']);
$statement->execute();
        ...
        <div class="container">
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
                ...
                for($i=0; $i<$numRows;$i++) {
                    ...
                    echo '<form action="editObject.php" method="get">';
                    echo '<div class="btn-group">';
                    echo '<input type="hidden" id="sId" name="sId" value=' . $daten[$i]->s_id . '>';
                    echo '<input type="submit" class="btn btn-sm btn-outline-secondary" value="Ändern"/>';
                    echo '</div>';
                    echo '</form>';
                    echo '<form name="deleteObject" action="deleteObject.php" method="post">';
                    echo '<input type="hidden" id="sId" name="sId" value=' . $daten[$i]->s_id . '>';
                    echo '<button class="btn btn-sm btn-outline-secondary" onclick="delObj()">Löschen</button>';
                    echo '</form>';
...
<script src="../script/delObj.js"></script>
</body>
</html>
